...
function ProductGrid({ items, showSale = false }) {
  return (
    <div className="grid grid-cols-3 gap-4 md:gap-6">
      {items.map((item) => (
        <Link
          key={item.name}
          to={showSale ? "/shop" : "/shop/new"}
...
export default function HomePage() {
  return (
    <>
      ...
      <section className="max-w-5xl mx-auto px-5 md:px-10 py-10 md:py-14">
        <SectionHeader title="NEW" linkTo="/shop/new" />
        <ProductGrid items={NEW_ITEMS} />
      </section>

      <section className="max-w-5xl mx-auto px-5 md:px-10 py-10 md:py-14 border-t border-[#e8e4df]">
        <SectionHeader title="SALE" linkTo="/shop" />
        <ProductGrid items={SALE_ITEMS} showSale />
      </section>
    </>
  );
}
